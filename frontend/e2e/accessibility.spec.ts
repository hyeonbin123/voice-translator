...
test('keyboard recording start, cancel, stop, upload chooser and long file name', async ({ page }) => {
  ...
  await activate(page, page.getByRole('button', { name: '번역하기' }))
  await expect(page.getByRole('heading', { name: '번역 결과' })).toBeFocused()
  await tabTo(page, page.getByLabel('또는 음성 파일 선택'))
  const chooser = page.waitForEvent('filechooser')
  await page.keyboard.press('Enter')
  // Only the operating system file picker is replaced; the input is opened with Enter.
  await (await chooser).setFiles({ name: `${'long-filename-'.repeat(20)}.wav`, mimeType: 'audio/wav', buffer: Buffer.from('recording') })
