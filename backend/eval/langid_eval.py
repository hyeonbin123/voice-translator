...
from eval.eos_eval import LANGS, RATE, load_clips, word_bounds
...
def pieces(clip: np.ndarray, start: int, end: int) -> dict[str, np.ndarray]:
    begin = max(0, start - MARGIN)
    return {
        name: clip[begin : (start + int(seconds * RATE)) if seconds else end + MARGIN]
    ...
    for lang in LANGS:
        ...
        for key, clip in clips:
            if not bounds.get(key):
                continue
            for length, piece in pieces(clip, *bounds[key]).items():
...
def write_report(guesses: list[dict], args: argparse.Namespace) -> None:
    conditions, rule = summarize(guesses, args.thresholds)
    gpu, _ = gpu_memory_mb()
    stamp = datetime.now(UTC)
    base = REPORTS / f"langid_{args.tag}_{stamp:%Y%m%d_%H%M%S}"
    report = {"gpu": gpu, "conditions": conditions, "rule": rule, "guesses": guesses}
